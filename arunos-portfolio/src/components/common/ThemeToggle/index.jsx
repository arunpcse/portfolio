...
const ThemeToggle = ({ className = '' }) => {
  ...
  return (
    <button onClick={toggleTheme} className={`p-2 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700 transition-all duration-200 ${className}`} title={isDark ? 'Switch to light mode' : 'Switch to dark mode'}>
      {isDark ? (
